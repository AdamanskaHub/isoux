<script>


</script>

<style>
  .bargrey{height:40px;width:80%;max-width: 200px;background-color: darkgray;margin-bottom: 20px;}
  .bargrey{height:40px; background-color: aquamarine;}
</style>


<h2 class="big-title mb-4">Website progress</h2>

<p class="opacity-50 mb-8">The whole plan, but more from a UX perspective, without getting into any technical parts.</p>
<p>To know the whys please <a href="https://docs.google.com/presentation/d/17LwRe2JXquj95R0EIYjfMoufuBCkVO95t0EtW9yVUZA/edit?usp=sharing">checkout the ppt</a>.</p>

<h3>Architecture</h3>
<p>Card sorting x2</p>
<div class="bargrey"><div class="bargreen w-full"></div></div>
<p>Brainstorming Lionel/David/Emilie</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Usability testing of the nav</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>

<h3>Sections</h3>
<p>Casco</p>
<div class="bargrey"><div class="bargreen w-full"></div></div>
<p>Gender</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>R&I</p>
<div class="bargrey"><div class="bargreen w-2/12"></div></div>
<p>Capacity</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Standard</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>TMB</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>About us</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Testing understanding</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>

<h3>UI (pure look&feel)</h3>
<p>Desirability tests</p>
<div class="bargrey"><div class="bargreen w-10/12"></div></div>
<p>Design system first draft figma</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Design system discussion David/emilie/Lionel</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Coded design system</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>


<h3>Writing</h3>
<p>Creating the topics pages content</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Tone/voice guidelines for the web</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Rewriting each section</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Case studies/guides & co</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>

<h3>SEO</h3>
<p>Find opportunities</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Find subjects to fight for ranking</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>
<p>Find subjects for writing articles, like keywords</p>
<div class="bargrey"><div class="bargreen w-0"></div></div>

